...
export function BoldTemplate({ business, services, hours, theme }: BoldTemplateProps) {
    ...
    <div className="min-h-screen font-sans" style={{ backgroundColor: bg, color: ink }}>
      ...
      <section className="relative h-80 w-full overflow-hidden md:h-96" style={{ backgroundColor: ink }}>
        ...
        <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-6 pt-5">
          <span className="text-xs font-semibold uppercase tracking-widest text-white/70">OYRB</span>
          <a
        ...
        <div className="absolute bottom-0 left-0 right-0 px-6 pb-8">
          <div className="flex items-end gap-4">
            ...
            >
              <Image src={unsplash(profileId, 160)} alt={bizName} fill className="object-cover" sizes="80px" />
            </div>
            <div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-white/70">
                <span className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} size={10} fill="gold" className="text-yellow-400" />
                  ))}
                  <span className="ml-1">5.0 (48 reviews)</span>
                </span>
        ...
        <div className="grid gap-8 md:grid-cols-[1fr_280px]">
          ...
          <main>
            <div className="mb-2 flex items-center justify-between">
              <h2 className="text-xl font-bold" style={{ fontFamily: displayFont }}>Services</h2>
            ...
            {galleryIds.length > 0 && (
              <section className="mt-10">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="text-xl font-bold" style={{ fontFamily: displayFont }}>Photos</h2>
                  <span className="text-xs" style={{ color: muted }}>{galleryIds.length} photos</span>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {galleryIds.slice(0, 6).map((id, i) => (
                    <div
                      key={i}
          ...
          <aside className="flex flex-col gap-4">
            ...
            <div className="p-5" style={{ borderRadius: radius, border: `1px solid ${border}`, backgroundColor: surface }}>
              <h3 className="mb-3 text-sm font-bold">Hours</h3>
            ...
            <div className="p-5" style={{ borderRadius: radius, border: `1px solid ${border}`, backgroundColor: surface }}>
              ...
              {[
                { text: "She's amazing! My hair has never looked better.", author: "Tasha M." },
                { text: "Booked same week, easy process, stunning results.", author: "Renee A." },
              ].map((r) => (
