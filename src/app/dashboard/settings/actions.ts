...
/**
 * @deprecated Superseded by `finalizeRemoval` in
 *   `src/lib/remove-brand/finalize.ts` (Phase 8 PR 5). The new
 *   function is cron-driven, per-business, runs a Storage cleanup
 *   pass, sends a finalization email, writes an audit row, and
 *   correctly handles multi-business users (only deletes
 *   auth.users on last-business finalization).
 *
 *   This function has zero callers as of Phase 8 PR 5. **Safe to
 *   delete in a future cleanup PR.** Kept temporarily for git-blame
 *   continuity — anyone tracing the destructive-delete code path
 *   lands here and finds the redirect to the new module.
 */
